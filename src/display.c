...
int
DrawBitmapP(bitmap_t* bm, int x, int y)
{
  int i;
  unsigned char* alias;
...
int
DrawBitmap(const char* name, int x, int y)
{
  bitmap_t* bm;
...
static void
FindDimPCX(FILE* fp, int* xres, int* yres)
{
  fseek(fp, 8, SEEK_SET);
...
static int
LoadPCX(FILE* fp, unsigned char* buffer, int xres, int yres)
{
  int i;
  int cur_byte;
  int run_len;
...
static int
RegisterBitmap(const char* name, const char* file)
{
  FILE* fp;
  bitmap_t* bm;
  int size;
