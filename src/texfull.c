    ...
    if (Textures[i].data)
    {
      for (j = 0; j < Textures[i].dsy; j++)
      {
        memcpy(&(video.ScreenBuffer[x1 + (y + j) * (video.ScreenWidth)]),
               &(Textures[i].data[leftadj + j * Textures[i].dsx]),
               sizeadj);
      }
    }
  }
...
int
TexPickFull(char* PickedName, int NumT, struct texture_s* Tex)
{
  int Redraw;
  char* OldScreen;
#ifndef WINDOW_MODE
  ...
  while (1)
  {
          ...
          else*/
    {
      for (i = 0; i < NumTextures; i++)
      {
        if (!Textures[i].data)
          continue;
        if (InBox(TexButtons[i].x - LeftX, TexButtons[i].y, TexButtons[i].x - LeftX + TexButtons[i].sx, TexButtons[i].y + TexButtons[i].sy))
        {
          BUTTONHIT = i;
          CurSelect = i;
          HitIt = TRUE;
    ...
    if (HitIt == TRUE)
    {
      if (CurSelect != OldSelect)
      {
        if (OldSelect != -1)
        {
          ...
          QUI_DrawStr(Textx1, Texty1, BG_COLOR, BG_COLOR, 0, TRUE, "%32s %ix%i", t->name, t->rsx, t->rsy);

          GetTexDesc(tdesc, t);
          QUI_DrawStr(Textx1 + 360, Texty1, BG_COLOR, BG_COLOR, 0, TRUE, "%s", tdesc);
        }

        t = &Textures[CurSelect];

        QUI_DrawStr(Textx1, Texty1, BG_COLOR, 15, 0, TRUE, "%32s %ix%i", t->name, t->rsx, t->rsy);

        GetTexDesc(tdesc, t);
        QUI_DrawStr(Textx1 + 360, Texty1, BG_COLOR, 15, 0, TRUE, "%s", tdesc);
