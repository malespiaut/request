...
AutoBuildPopup(void)
{
  ...
  for (i = 0; i < n_bcfg; i++)
    QUI_DrawStrM(w->pos.x + 45, w->pos.y + 40 + 22 * i, w->pos.x + w->size.x - 4, BG_COLOR, 0, 0, FALSE, "%s", bcfg[i].desc);
