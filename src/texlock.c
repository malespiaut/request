...
static void
TextureAxisFromPlane(plane_t* pln, float xv[3], float yv[3])
{
  int bestaxis;
  float dot, best;
  int i;
...
static int
orientation(float p[2], float q[2], float r[2])
{
  int i;
  float a[2], b[2];
...
*/
static void
Solution(brush_t* orig_b, plane_t* orig_p, brush_t* new_b, plane_t* new_p)
{
  ...
  */
  ...
  new_p->tex.rotate = angle(new_st[0], new_st[1]) - angle(orig_st[0], orig_st[1]);

  if (orientation(orig_st[0], orig_st[1], orig_st[2]) == orientation(new_st[0], new_st[1], new_st[2]))
  {
  ...
  */
  ...
  for (j = 0; j < 2; j++)
  {
    ...
    {
      p = fabs(
        (new_st[pset[i][0]][j] - new_st[pset[i][1]][j]) * (orig_st[pset[i][0]][j] - orig_st[pset[i][1]][j]));
    ...
    {
      new_p->tex.scale[j] =
        (new_st[pset[best][0]][j] - new_st[pset[best][1]][j]) / (orig_st[pset[best][0]][j] - orig_st[pset[best][1]][j]);
    }
    else
    {
      new_p->tex.scale[j] = 1;
