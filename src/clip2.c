...
int AddBrushes(brush_t *b1, brush_t *b2)
{
	int             i,j;
	brush_my_t A;
	brush_my_t B;
...
static void CSG_MakeHollow(brush_t *b)
{
   plane_t *p;
   int i,j;
   brush_t *temp;
...
static void CSG_MakeRoom(brush_t *b)
{
   plane_t *p;
   int i,j;
   brush_t *temp;
...
static int ClipBrush(brush_t *b,plane_t *p,int flip)
{
   int j;
   brush_t *temp;
   brush_my_t t;
...
//		ClearKeys();
   ...
   for (br=br1;br;br=br->Next)
   {
      ...
      if (!ClipBrush(br->Brush,p,0)) error++;
      if (!ClipBrush(br->Brush,p,1)) error++;
...
int JoinBrushes(brush_t *b1, brush_t *b2)
{
	int     i,j,k,l;
	brush_my_t A;
	brush_my_t B;
