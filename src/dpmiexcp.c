...
do_faulting_finish_message(void)
{
  ...
  en = (signum >= EXCEPTION_COUNT) ? 0 : exception_names[signum];
  if (signum == 0x75)
  ...
  err("\r\neax=");
  itox(__djgpp_exception_state->__eax, 8);
  err(" ebx=");
  itox(__djgpp_exception_state->__ebx, 8);
  err(" ecx=");
  itox(__djgpp_exception_state->__ecx, 8);
  err(" edx=");
  itox(__djgpp_exception_state->__edx, 8);
  err(" esi=");
  itox(__djgpp_exception_state->__esi, 8);
  err(" edi=");
  itox(__djgpp_exception_state->__edi, 8);
  err("\r\nebp=");
  itox(__djgpp_exception_state->__ebp, 8);
  err(" esp=");
  itox(__djgpp_exception_state->__esp, 8);
  err(" program=");
...
__djgpp_exception_setup(void)
{
  ...
  lockmem.size = ((unsigned)&__djgpp_hw_lock_end - (unsigned)&__djgpp_hw_lock_start);
...
_exit(int status)
{
  ...
  if (__djgpp_old_kbd.offset32 == kbd_ori.offset32 && __djgpp_old_kbd.selector == kbd_ori.selector)
