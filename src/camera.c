...
void
Move(int vport, int dir, int* dx, int* dy, int* dz, int amt)
{
  matrix_t m;
  scalar_t d[3], e[3];
  int i;
...
void
Move90(int vport, int dir, int* dx, int* dy, int* dz, int amt)
{
  matrix_t m;
  scalar_t d[3], e[3];
  int i;
...
LookDown(int vport)
{
  ...
  if (M.display.vport[vport].axis_aligned)
  {
    ...
    else if ((M.display.vport[vport].camera_dir & 0xF8) == 0)
...
TurnLeft(int vport)
{
  ...
  if (M.display.vport[vport].axis_aligned)
  {
    ...
    else if ((M.display.vport[vport].camera_dir & 0x03) == LOOK_POS_Y)
    {
      M.display.vport[vport].camera_dir &= 0xF8;
      M.display.vport[vport].camera_dir |= LOOK_NEG_X;
    }
    else if ((M.display.vport[vport].camera_dir & 0x03) == LOOK_NEG_X)
    {
      M.display.vport[vport].camera_dir &= 0xF8;
      M.display.vport[vport].camera_dir |= LOOK_NEG_Y;
    }
    else if ((M.display.vport[vport].camera_dir & 0x03) == LOOK_NEG_Y)
    {
      M.display.vport[vport].camera_dir &= 0xF8;
...
InitCamera(void)
{
  ...
  for (e = M.EntityHead; e && (!found); e = e->Next)
  {
    ...
    found = TRUE;
    sscanf(GetKeyValue(e, "origin"), "%d %d %d", &origin.x, &origin.y, &origin.z);
