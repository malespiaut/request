...
static inline int ToScreen(svec_t *d,vec3_t s)
{
   switch (cur_vp->mode)
   {
      case NOPERSP:
...
static void DrawGrid(int snap,int col)
{
   int x1;
   int mx,my,mz;
   int ofs;
...
static int OutsideFrustrum(vec3_t from, vec3_t to)
{
   if ((from.z <= 0)      && (to.z <= 0))    return TRUE;
   if ((from.x >  from.z) && (to.x >  to.z)) return TRUE;
   if ((from.x < -from.z) && (to.x < -to.z)) return TRUE;
   if ((from.y >  from.z) && (to.y >  to.z)) return TRUE;
   if ((from.y < -from.z) && (to.y < -to.z)) return TRUE;
   return FALSE;
}


static void DrawBrushFaceEdges(brush_t *b,int face,int base_color,int force_bright)
{
	int   color;
	int   from, to;
	float  x0, y0, x1, y1;
	...
	for (i=0; i<b->num_edges;i++)
	{
		...
		switch (cur_vp->mode)
		{
         ...
         case SOLID:
            ...
            if (OutsideFrustrum(tverts[from],tverts[to])) continue;
...
static void DrawBrushFace(brush_t *b,int face,int base_color,int force_bright)
{
...
static void DrawBrushEdges(int vport,brush_t *b,int base_color,int selected)
{
...
void DrawBrush(int vport,brush_t *b,int color,int selected)
{
   if (b->bt->flags&BR_F_EDGES)
   {
...
static void DrawPts(matrix_t m)
{
   int i;
   ...
   for (i=0;i<M.npts-1;i++)
   {
      if (status.draw_pts==3)
      {
         ...
         if (s.onscreen)
         {
            ...
            x=s.x-32;
            if (x<cur_vp->xmin+1) x=cur_vp->xmin+1;
            
            y=s.y-32;
            if (y<cur_vp->ymin+1) y=cur_vp->ymin+1;
      ...
      if (status.draw_pts==2)
      {
         ...
         ToScreen(&s,M.tpts[i]);
         if (s.onscreen) DrawVertex(cur_vport,s,CR_VERT);
      }
      else
      {
         if (i==(M.npts-2))
		...
		switch (cur_vp->mode)
		{
         ...
         case SOLID:
            ...
            if (OutsideFrustrum(M.tpts[i],M.tpts[i+1])) continue;

				if (!cur_vp->fullbright)
				{
					if ((M.tpts[i].z > 1) && (M.tpts[i+1].z > 1)) continue;
				}
...
static void DrawViewport(matrix_t m)
{
	int         i, j;
   int         axis_x,axis_y;
	vec3_t      origin;
	...
	for (b=M.BrushHead; b; b=b->Next)
	{
      ...
      if ((status.edit_mode == ENTITY) || (status.edit_mode == MODEL))
      {
         ...
         {
#define CheckMin(x) \
   if (b->verts[i].##x<e->min.##x) e->min.##x=b->verts[i].##x
#define CheckMax(x) \
   if (b->verts[i].##x>e->max.##x) e->max.##x=b->verts[i].##x
            CheckMin(x);
            CheckMin(y);
            CheckMin(z);
            CheckMax(x);
		...
		case WIREFRAME:
         ...
         for (b=M.BrushHead; b; b=b->Next)
         {
			...
			switch (status.edit_mode)
			{
               ...
               for (fsel=M.display.fsel; fsel; fsel=fsel->Next)
               {
               	DrawDot(cur_vport,fsel->Brush->plane[fsel->facenum].scenter,
                          CR_FACE_SELECT);
               }
...
/*      { // TODO: clamp framerate in a smarter way
		...
		RefreshPart(cur_vp->xmin,cur_vp->ymin,
				      cur_vp->xmax,cur_vp->ymax);
   }
}
