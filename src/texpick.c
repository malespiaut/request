...
static int
mlsort(const void* e1, const void* e2)
{
  return Game.tex.sort(*(char* const*)e1, *(char* const*)e2);
}

static int
ReadMainList(int cache)
{
  int i;
...
static int
clsort(const void* e1, const void* e2)
{
...
DrawCList(void)
{
  ...
  for (i = 0; i < rows; i++)
  {
    ...
    QUI_DrawStrM(cx1 + 2, y1 + i * ROM_CHAR_HEIGHT, cx2 - 2, BG_COLOR, col, 0, 0, "%s", clist[i + cy].name);
  }
}

static void
DrawDList(QUI_window_t* w)
{
  int i, col;
  texture_t* t;
  char tdesc[128];
  ...
  for (i = 0; i < rows; i++)
  {
    ...
    QUI_DrawStrM(dx1 + 2, y1 + i * ROM_CHAR_HEIGHT, dx2, BG_COLOR, col, 0, 0, "%s", curlist[i + dy]);
  }
  ...
  if (tex >= 0 && tex < ncl)
  {
    ...
    QUI_DrawStr(dx2 + 24, y2 - 4 - ROM_CHAR_HEIGHT * 3, BG_COLOR, 0, 0, 0, "%s", curlist[tex]);

    if (curtex.dsx != -1)
    {
      DrawTexture(&curtex, dx2 + 24, y1);
      QUI_DrawStr(dx2 + 24, y2 - 4 - ROM_CHAR_HEIGHT * 2, BG_COLOR, 0, 0, 0, "%3i x %3i", curtex.rsx, curtex.rsy);

      GetTexDesc(tdesc, &curtex);
      QUI_DrawStr(dx2 + 24, y2 - 4 - ROM_CHAR_HEIGHT, BG_COLOR, 0, 0, 0, "%s", tdesc);
    }
  }
...
void
TexturePicker(const char* StartWithThis, char* EndWithThis)
{
  QUI_window_t* w;
