...
void
gui_button::Init(int flags, char* text)
{
  FUNC
...
void
gui_button::InitP(int flags, char* pic)
{
  FUNC
  ...
  switch (bType)
  {
    ...
    case btntText:
      ...
      if (bState == btsDisabled)
        c1 = 8;
      else if (bState == btsFocused)
        c1 = 15;
      else if (bFlags & btnDefault)
        c1 = COL_YELLOW;
      else
        c1 = 13;

      QUI_DrawStrM(x1, y1 + 1, rx2, GetColor(BG_COLOR), GetColor(c1), 0, 0, (char*)bData);
      break;
  }
...
void
gui_button::HandleEvent(event_t* e)
{
  FUNC
...
void
gui_button::SetState(int newstate)
{
  FUNC
