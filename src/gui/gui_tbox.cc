...
void
gui_textbox::Init(unsigned int maxlen, char* default_text)
{
  FUNC
...
gui_textbox::Draw(void)
{
  ...
  if (*c)
  {
    QUI_DrawChar(rx2 - 2 - f->width[f->map[(int)'>']], y, GetColor(15), GetColor(COL_GREEN - 2), 0, 0, '>');
  }
...
void
gui_textbox::InsertStr(int len, char* str)
{
  FUNC
...
int
gui_textbox::SkipWord(int start, int dir)
{
  start += dir;
  if (start < 0)
    return 0;
...
void
gui_textbox::HandleEvent(event_t* e)
{
  FUNC
...
void
gui_textbox::SetText(char* ntext)
{
  strcpy(text, ntext);
  sel_start = sel_end = -1;
  cur_pos = cur_inc = 0;
...
void
gui_textbox_int::Init(int startvalue)
{
  char buf[128];
...
void
gui_textbox_int::SetValue(int value)
{
  char buf[128];
...
void
gui_textbox_float::Init(float startvalue)
{
  char buf[128];
...
void
gui_textbox_float::SetValue(float value)
{
  char buf[128];
...
float
gui_textbox_float::GetValue(void)
{
  value = atof(text);
  return value;
}
...
void
gui_textbox_float::FocusOff(void)
{
  char buf[128];
