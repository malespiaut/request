...
static int
MakeUniqueFaceVerts(bsp_face_t* F)
{
  int i, endnumber;
...
static int
AddToTree(bsp_face_t* F, bsp_node_t* Root, int onnode)
{
  bsp_face_t* Front;
  bsp_face_t* Back;
  int i;
  ...
  switch (i)
  {
    ...
    case -1:
      if (Root->BackNode != NULL)
        return AddToTree(F, Root->BackNode, 0);
    ...
    case -2:
      if (Root->FrontNode != NULL)
        return AddToTree(F, Root->FrontNode, 0);
    ...
    case 0:
      ...
      if (!AddToTree(Back, Root->BackNode, 0))
        return FALSE;
      ...
      if (!AddToTree(Front, Root->FrontNode, 0))
        return FALSE;
...
static void
TextureAxisFromPlane(bsp_plane_t* pln, float vecs[2][3])
{
...
static bsp_map_face_t*
NewMF(brush_t* b, plane_t* p)
{
  bsp_map_face_t* temp;
  bsp_map_face_t** t;
...
DeleteBSPNode(bsp_node_t* Root)
{
  ...
  if (Root->faces != NULL)
  {
    for (i = 0; i < Root->numfaces; i++)
    {
      ...
      {
        if (Root->faces[i]->pts != NULL)
          Q_free(Root->faces[i]->pts);
        if (Root->faces[i]->i_pts != NULL)
          Q_free(Root->faces[i]->i_pts);
        if (Root->faces[i]->t_pts != NULL)
          Q_free(Root->faces[i]->t_pts);
        Q_free(Root->faces[i]);
      }
    }
    Q_free(Root->faces);
