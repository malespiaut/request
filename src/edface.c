...
static fsel_t *SelectFace(int vport, int mx, int my)
{
   fsel_t *list,*l;
   int n_list;
...
static void SelectWindowFace(int vport, int x0, int y0, int x1, int y1)
{
	int      numnew;
	int      i;
	int      x2, y2;
...
static void SelectWindowFaceVert(int vport, int x0, int y0, int x1, int y1)
{
	int      numnew;
	int      i, j;
	fsel_t  *f;
...
static vsel_t *FindFaceVertex(int vport, int mx, int my)
{
	int      j;
	vsel_t   best;
	float    best_val;
		...
		if (sel_face != NULL)
		{
			...
			for (f=M.display.fsel; f; f=f->Next)
			{
				if ((f->Brush == sel_face->Brush) && (f->facenum == sel_face->facenum))
				{
					/* Removing */
					found = TRUE;
					if (f->Last != NULL) f->Last->Next = f->Next;
					if (f->Next != NULL) f->Next->Last = f->Last;
					if (f == M.display.fsel) M.display.fsel = f->Next;
					Q_free(f);
               Q_free(sel_face);
   				M.display.num_fselected--;
					break;
