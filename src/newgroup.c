...
static void
BubbleSort(char list[200][50], int* ref, int num)
{
  int i;
  char temp[256];
  int stemp;
...
GroupPopup(void)
{
  ...
  void ReDraw(void)
  {
    ...
    for (i = base_group, j = 0; (j < 17) && (i < M.num_groups); i++, j++)
      QUI_DrawStr(w->pos.x + 75, w->pos.y + 52 + (j * 14), BG_COLOR, (i == active_group) ? 14 : 0, 0, FALSE, group_list[i]);
  ...
  while (1)
  {
    ...
    if (bp != -1)
    {
      ...
      if (bp == b_delete)
      {
        ...
        else if (strcmp(group_list[active_group], "World") == 0)
        {
