...
static void
AddTNameDir_r(char* dir, const char* base, int nl)
{
...
*/
void
PD_Search(const char* p_dir, const char* p_ext, void (*p_addtname)(char* name, int nl, int ofs))
{
  char buf[256];
  char *c, *d;
  location_t* l;
    ...
    switch (l->type)
    {
      case LOC_DIR:
        if (!AddTNameDir(n_locs - 1))
          Abort("PD_Init", "Unable to load textures from '%s'!", l->name);
        break;
      case LOC_PAK:
...
FILE*
PD_Load(tex_name_t* tn, int* baseofs, int* closefile)
{
  location_t* l;
  char name[512];
  FILE* f;
