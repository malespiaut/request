...
static int Q_TexSort(const char *t1,const char *t2)
{
...
static int Q_ReadCache(int verbose)
{
   FILE *f;
   int i;
   int flag;
...
static void Q_FlagsDefault(texdef_t *t)
{
   t->g.q.detail=0;
}


static int Q_TexBSPFlags(texdef_t *t)
{
   if (t->name[0]=='*')             return TEX_FULLBRIGHT|TEX_NONSOLID;
   if (!strnicmp(t->name,"SKY",3))  return TEX_FULLBRIGHT;
   if (!stricmp(t->name,"TRIGGER")) return TEX_NODRAW;
   if (!stricmp(t->name,"CLIP"))    return TEX_NODRAW|TEX_NONSOLID;
   return 0;
}
...
static int Q_LoadTexInfo(texdef_t *t)
{
   int i;
...
static void Q_SaveTexInfo(texdef_t *t,FILE *fp)
{
...
static void QI_AddTName(char *name,int nl,int ofs)
{
   tex_name_t *t;
   int i;
...
static int Q_I_LoadTexture(tex_name_t *tn)
{
   FILE *f;
   int baseofs;
   int closefile;
