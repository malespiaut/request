...
static int Q3_LoadTexInfo(texdef_t *tex)
{
   int i;
...
static void Q3_SaveTexInfo(texdef_t *tex,FILE *fp)
{
...
static void Q3_FlagsDefault(texdef_t *tex)
{
   tex->g.q2.contents=tex->g.q2.flags=tex->g.q2.value=0;
}


static int Q3_TexSort(const char *t1,const char *t2)
{
   return strcmp(t1,t2);
}
...
static int Q3_TexBSPFlags(texdef_t *t)
{
   int i;
   tex_name_t *tn;
   q3_tn_int_t *sh;
   ...
   if (sh->flags&SHADER_SKY)       return TEX_FULLBRIGHT;

   i=0;
   if (sh->flags&SHADER_NODRAW)    i|=TEX_NODRAW;
   if (sh->flags&
       (SHADER_WATER|SHADER_SLIME|SHADER_LAVA|SHADER_FOG
       |SHADER_NOTSOLID|SHADER_TRANS)
      )
...
static void Q3_TextureDesc(char *dest,const texture_t *t)
{
   int i;
   tex_name_t *tn;
   q3_tn_int_t *sh;
...
static texture_t *Q3_LoadTGA(tex_name_t *real,tex_name_t *tn)
{
   FILE *f;
...
static int Q3_LoadTexture(tex_name_t *tn)
{
   tex_name_t *real;
   texture_t *t;
...
static void AddShader(char *name,int ref,q3_tn_int_t *sh)
{
   tex_name_t *t;
   int i;
...
static int LookupImage(const char *iname)
{
   const char *in;
   int len;
   int i;
...
static void LoadShaderFile(char *fname)
{
   char name[256];
   int brace_level;
   int i;
...
/* general shader information */
         ...
         if (!strcmp(token,"surfaceparm"))
         {
            ...
            if (!strcmp(token,"nonsolid"))
               sh.flags|=SHADER_NOTSOLID;
            else
            if (!strcmp(token,"trans"))
               sh.flags|=SHADER_TRANS;
               
            else
            if (!strcmp(token,"lava"))
               sh.flags|=SHADER_LAVA;
            else
            if (!strcmp(token,"slime"))
         ...
         if (!strcmp(token,"q3map_surfacelight"))
         {
            TokenGet(1,-1);
            sh.light=atof(token);
         }
         ...
         if (!strcmp(token,"qer_editorimage"))
         {
            TokenGet(1,-1);
...
static int Q3_GetTexCat(char *name)
{
   char *c;
   int i,j;
   category_t *t;
   ...
   {
      if (!strnicmp(&t->name[2],name,j)) return i;
   }
   ...
   n_tcategories=0;
   {
      ...
      for (i=0,t=tnames;i<n_tnames;i++,t++)
      {
         ...
         for (j=0,ct=tcategories;j<n_tcategories;j++,ct++)
            if (!strnicmp(&ct->name[2],t->name,k)) break;
         if (j<n_tcategories) continue;
         tcategories=Q_realloc(tcategories,sizeof(category_t)*(j+1));
         if (!tcategories) Abort("Q3_Init","Out of memory!");
         ct=&tcategories[j++];
         strcpy(ct->name,"- ");
