...
char**
LoadModelNames(int types, int* numnames)
{
  char** MDLNames = NULL;
  int i;
  int numlines = 0;
...
void
DrawModel(int vport, entity_t* ent, matrix_t m, int selected)
{
  int i;
  vec3_t rp, tpos;
  float cosang, sinang;
  ...
  else if (M.display.vport[vport].mode == WIREFRAME ||
           M.display.vport[vport].mode == SOLID)
  {
    for (i = 0; i < mdl.numedges; i++)
    {
      ClipDrawLine3D(vport, mdl.tvertlist[mdl.edgelist[i].startvertex].x, mdl.tvertlist[mdl.edgelist[i].startvertex].y, mdl.tvertlist[mdl.edgelist[i].startvertex].z, mdl.tvertlist[mdl.edgelist[i].endvertex].x, mdl.tvertlist[mdl.edgelist[i].endvertex].y, mdl.tvertlist[mdl.edgelist[i].endvertex].z, color);
    }
  }
...
static FILE*
FindInPak(char* name, int* baseofs)
{
  int i;
  FILE* f;
