...
static void
AddPoint(face_my_t* F, vec3_t pt)
/*
...
static int
RemoveFace(brush_my_t* B, int pos)
/*
...
int
DivideFaceByPlane(face_my_t F, plane_my_t P, face_my_t* Front, face_my_t* Back, int KeepBack)
/*
...
*/
{
  ...
  for (i = 0; i < F.numpts; i++)
  {
    ...
    {
      tfpts++;
      if (tfpts > MAX_FACE_POINTS)
        return -3;
      AddPoint(&FrontFace, p1);
      if (KeepBack)
      {
        tbpts++;
        if (tbpts > MAX_FACE_POINTS)
          return -3;
  ...
  for (i = 0; i < Front->numpts; i++)
    Front->pts[i] = FrontFace.pts[i];
  ...
  if (KeepBack)
  {
    ...
    for (i = 0; i < Back->numpts; i++)
      Back->pts[i] = BackFace.pts[i];
...
static void
VectorMA(qbsp_vec3_t va, float scale, qbsp_vec3_t vb, qbsp_vec3_t vc)
{
...
static void
CrossProduct(qbsp_vec3_t v1, qbsp_vec3_t v2, qbsp_vec3_t cross)
{
...
static float
DotProduct(qbsp_vec3_t v1, qbsp_vec3_t v2)
{
  return v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2];
}

static void
VectorSubtract(qbsp_vec3_t va, qbsp_vec3_t vb, qbsp_vec3_t out)
{
  out[0] = va[0] - vb[0];
  out[1] = va[1] - vb[1];
  out[2] = va[2] - vb[2];
...
static void
VectorAdd(qbsp_vec3_t va, qbsp_vec3_t vb, qbsp_vec3_t out)
{
  out[0] = va[0] + vb[0];
  out[1] = va[1] + vb[1];
  out[2] = va[2] + vb[2];
...
static void
VectorScale(qbsp_vec3_t v, float scale, qbsp_vec3_t out)
{
  out[0] = v[0] * scale;
  out[1] = v[1] * scale;
  out[2] = v[2] * scale;
...
int
BuildBrush(brush_my_t* B, brush_t* b)
{
  edge_t tmpedge;
  int i, j, k, l;
  int flag;
...
brush_t*
MyBrushIntoMap(brush_my_t* B, entity_t* EntityRef, group_t* Group)
{
  edge_t tmpedge;
  int i, j, k, l;
  int flag;
...
int
BrushesIntersect(brush_my_t* b1, brush_my_t* b2)
{
  int i, j, k;
  face_my_t Face1, Face2;
  int totalfront;
...
static int
SplitBrushWithPlane(brush_my_t* Bfront, face_my_t P, brush_my_t* Bback)
/* Returns:
...
*/
{
  ...
  if ((ontest == 0) && (fronttest == 0) && (backtest == 0))
    return -3;
...
static int
SubtractBrushes(brush_t* b1, brush_t* b2)
/*
   Returns:  0 = succesful
