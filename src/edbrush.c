...
int
AddSelBrush(brush_t* sel_brush, int addgroup)
{
  brushref_t* b;
  brush_t* br;
...
void
SelectWindowBrush(int x0, int y0, int x1, int y1)
{
  int numnew;
  int i;
  int x2, y2;
...
void
ApplyTexture(brush_t* b, char* texname)
{
  int i;
...
AutoZoom(void)
{
  ...
  for (b = M.display.bsel; b; b = b->Next)
  {
    for (i = 0; i < b->Brush->num_verts; i++)
    {
      if (b->Brush->verts[i].x > xmax)
        xmax = b->Brush->verts[i].x;
      if (b->Brush->verts[i].x < xmin)
        xmin = b->Brush->verts[i].x;

      if (b->Brush->verts[i].y > ymax)
        ymax = b->Brush->verts[i].y;
      if (b->Brush->verts[i].y < ymin)
        ymin = b->Brush->verts[i].y;

      if (b->Brush->verts[i].z > zmax)
        zmax = b->Brush->verts[i].z;
      if (b->Brush->verts[i].z < zmin)
        zmin = b->Brush->verts[i].z;
    }
  }
