...
static int
IsOnPlane(vec3_t norm, float d, vec3_t pt)
{
...
static void
Check_AddErrorB(check_error_type_t* t, brush_t* b)
{
  check_error_t* ce;
...
static void
Check_AddErrorE(check_error_type_t* t, entity_t* e)
{
  check_error_t* ce;
...
static void
CE_SetOrigin(check_error_t* e)
{
  SetKeyValue(e->x.e, "origin", "0 0 0");
}

static void
Check_Entity(entity_t* e)
{
  static check_error_type_t
    err_no_origin =
  ...
  while (1)
  {
    if (draw)
    {
      ...
      for (ce = &cerrors[sy], i = 0; i < 10; i++, ce++)
      {
        ...
        QUI_DrawStrM(l_x1 + 2, l_y1 + 2 + 16 * i, l_x2 - 164, BG_COLOR, j, 0, 0, "%s", ce->t->msg);

        switch (ce->t->type)
        {
          case CHK_BRUSH:
            QUI_DrawStrM(l_x2 - 162, l_y1 + 2 + 16 * i, l_x2 - 2, BG_COLOR, j, 0, 0, "b(%0.0f %0.0f %0.0f)", ce->x.b->center.x, ce->x.b->center.y, ce->x.b->center.z);
            break;
          case CHK_ENTITY:
            if (GetKeyValue(ce->x.e, "origin"))
              QUI_DrawStrM(l_x2 - 162, l_y1 + 2 + 16 * i, l_x2 - 2, BG_COLOR, j, 0, 0, "%s at %s", GetKeyValue(ce->x.e, "classname"), GetKeyValue(ce->x.e, "origin"));
            else
              QUI_DrawStrM(l_x2 - 162, l_y1 + 2 + 16 * i, l_x2 - 2, BG_COLOR, j, 0, 0, "%s", GetKeyValue(ce->x.e, "classname"));
            break;
        }
      }
    ...
    if (bp == b_delete)
    {
      ...
      else if (cerrors[y].t->type == CHK_BRUSH)
      {
...
int
CheckBrush(brush_t* b, int verbose)
{
  int i, j, k;
  vec3_t normal;
  vec3_t delta;
