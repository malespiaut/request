...
static int TextureAxisFromPlane(plane_t *pln,vec3_t *norm,vec3_t *xv,vec3_t *yv)
{
	int	bestaxis;
	float	dot,best;
	int	i;
...
static void DrawTexFacePoly(texture_t *texture)
{
	int     y, lp, rp, nlp, nrp;
	float   xl, xr;
	float   dxl, dxr;
...
nodraw:
   ...
   for (i=0;i<num_pts;i++)
   {
      ...
      if (pts[i].rtx<0) pts[i].rtx+=texture->rsx;
      if (pts[i].rty<0) pts[i].rty+=texture->rsy;

      QUI_DrawStr(pts[i].sx,pts[i].sy,-1,15,0,0,
         "%i,%i",pts[i].rtx,pts[i].rty);
   }
/*   for (;i<num_pts+4;i++)
   {
      pts[i].tx%=texture->sx;
...
static void DrawFacePoly(QUI_window_t *w,texdef_t *tex,texture_t *texture)
{
   float  shift[2];
   float  vecs[2][3];
...
static void DrawFaceWire(fsel_t *fs)
{
   brush_t *b;
   plane_t *p;
   int x[64];
   ...
   {
      n=i+1;
      if (n==p->num_verts) n=0;
      DrawLine(x[i],y[i],x[n],y[n],GetColor2(15));
   }
}

static void RedrawFacePopup(QUI_window_t *w,texdef_t *tex,texture_t *texture)
{
   int   i;
   fsel_t *fs;
   ...
   QUI_Box(w->pos.x+14,w->pos.y+39,w->pos.x+16+twsx,w->pos.y+41+twsy,
      4,8);
	...
	for (i=0; i<5; i++)
	{
      QUI_Box(w->pos.x+twsx+ 50,w->pos.y+48+i*40,
              w->pos.x+twsx+133,w->pos.y+66+i*40,
              4,8);
	}
...
static void AutoAlign(texdef_t *tex,texture_t *t,int xyb)
{
   vec3_t min;
   vec3_t pt;
   int i;
   ...
   if (xyb&1)
   {
      ...
      if (tex->shift[0]<0) tex->shift[0]+=t->rsx;
   }
   ...
   if (xyb&2)
   {
      ...
      if (tex->shift[1]<0) tex->shift[1]+=t->rsy;
   }
}

static void AutoScale(texdef_t *tex,texture_t *t,int xyb)
{
   vec3_t min,max;
   vec3_t p[3];
   vec3_t pt[3];
   ...
   for (i=0;i<num_pts;i++)
   {
      ...
      if (tmp<min.x) min.x=tmp;
      if (tmp>max.x) max.x=tmp;

      tmp=pts[i].x*yv.x+pts[i].y*yv.y+pts[i].z*yv.z;
      if (tmp<min.y) min.y=tmp;
...
start:
...

   {
      ...
      {
   #define ChkVec(a) \
      if (pts[i].a<fmin.a) fmin.a=pts[i].a;\
      if (pts[i].a>fmax.a) fmax.a=pts[i].a;
         ChkVec(x);
         ChkVec(y);
         ChkVec(z);
      }
   ...
   REDRAW();
	...
	while (!exit_flag)
	{
      ...
      else
      {
		...
		{
         while (TestKey(KEY_ESCAPE)) UpdateMouse();
			exit_flag = TRUE;
		}
		if (TestKey(KEY_ENTER))
      {
         ...
         while (TestKey(KEY_ENTER)) UpdateMouse();
      ...
      bp=UpdateButtons();
		...
		if (bp!=-1)
		{
         ...
         if (bp==b_flip)
         {
            ...
            if (i)
            {
               if (i&1) tex.scale[0]=-tex.scale[0];
               if (i&2) tex.scale[1]=-tex.scale[1];
               REDRAW();
            }
         }
      ...
      else
      {
         if (mouse.button==1)
         {
         	for (i=0; i<5; i++)
         	{
               if (InBox(w->pos.x+twsx+ 50,w->pos.y+48+i*40,
                         w->pos.x+twsx+133,w->pos.y+66+i*40))
                  break;
         	}
            if (i<5)
            {
               ...
               if (readstring(buf,w->pos.x+twsx+55,w->pos.y+50+i*40,w->pos.x+twsx+133,
                   sizeof(buf),NULL))
               {
                  *f=atof(buf);
               }
               REDRAW();
               while (TestKey(KEY_ENTER) || TestKey(KEY_ESCAPE)) UpdateMouse();
            }
         }
      }
	}
...
static void Curve_FitEven(brush_t *b)
{
   float *dist,*d;
   float x,y,z,v,e,f;
   int nx,ny;
...
static void EditFace_Curve(void)
{
   ...
   PushButtons();
	b_ok=AddButtonText(0,0,0,"OK");
	MoveButton(b_ok,w->pos.x+8,
	           w->pos.y+w->size.y-30);
              
	b_reset=AddButtonText(0,0,0,"Reset");
	MoveButton(b_reset,button[b_ok].x+button[b_ok].sx+4,
	           button[b_ok].y);
              
	b_fit=AddButtonText(0,0,0,"Fit");
	MoveButton(b_fit,button[b_reset].x+button[b_reset].sx+4,
	           button[b_ok].y);

	b_natural=AddButtonText(0,0,0,"Natural");
	MoveButton(b_natural,button[b_fit].x+button[b_fit].sx+4,
	           button[b_ok].y);

	b_even=AddButtonText(0,0,0,"Even");
	MoveButton(b_even,button[b_natural].x+button[b_natural].sx+4,
	           button[b_ok].y);
   ...
   redraw=1;
	...
	while (1)
	{
      if (redraw)
      {
         ...
         for (i=1;i<nx;i++) DrawLine(x1[i]-4,y1[0],x1[i]-4,y2[nx*ny-1],0);
         for (i=nx;i<ny*nx;i+=nx) DrawLine(x1[0],y1[i]-4,x2[nx-1],y1[i]-4,0);
