...
void
DrawDot(int vport, svec_t v, int color)
{
  unsigned char* ptr;
...
void
DrawBox(int vport, int x, int y, int size, int color)
{
  int i;
...
void
DrawSolidBox(int x1, int y1, int x2, int y2, int color)
{
  int i, j;
...
void
DrawSolidSquare(int x, int y, int size, int color)
{
  DrawSolidBox(x, y, x + size, y + size, color);
}

void
DrawVertex(int vport, svec_t v, int color)
{
  unsigned char* ptr;
...
void
ClipDrawLine2D(int vport_num, int x0, int y0, int x1, int y1, int color)
{
  int x2, y2;
  int c0, c1;
...
restart_clip3d:
  ...
  {
    TransformLineToScreen3D(
      vport_num, x0, y0, z0, x1, y1, z1, px0, py0, px1, py1);
    return 1;
  }
  else if ((c0 & c1) != 0)
  {
  ...
  else
  {
    if (c0 != 0)
    {
      ...
      if (c0 & 0x00000001)
        t = (-x0 + z0) / (dx - dz);
      else if (c0 & 0x00000002)
        t = (x0 + z0) / (-dx - dz);
      else if (c0 & 0x00000004)
        t = (-y0 + z0) / (dy - dz);
      else if (c0 & 0x00000008)
        t = (y0 + z0) / (-dy - dz);
      else if (c0 & 0x00000010)
        t = (1 - z0) / (dz);
      else if (c0 & 0x00000020)
        t = (z0) / (-dz);
      x0 = x0 + (t * dx);
      y0 = y0 + (t * dy);
      z0 = z0 + (t * dz);
    ...
    else
    {
      ...
      if (c1 & 0x00000001)
        t = (-x0 + z0) / (dx - dz);
      else if (c1 & 0x00000002)
        t = (x0 + z0) / (-dx - dz);
      else if (c1 & 0x00000004)
        t = (-y0 + z0) / (dy - dz);
      else if (c1 & 0x00000008)
        t = (y0 + z0) / (-dy - dz);
      else if (c1 & 0x00000010)
        t = (1 - z0) / (dz);
      else if (c1 & 0x00000020)
        t = (z0) / (-dz);
      x1 = x0 + (t * dx);
      y1 = y0 + (t * dy);
      z1 = z0 + (t * dz);
...
void
ClipDrawLine3D(int vport_num, float x0, float y0, float z0, float x1, float y1, float z1, int color)
{
  int dx0, dy0, dx1, dy1;
...
void
Clip2D(int vport_num, int x0, int y0, int x1, int y1, int* x2, int* y2, int code)
{
  // float t;
  float dx, dy;
...
void
Clip3D(float x0, float y0, float z0, float x1, float y1, float z1, float* x2, float* y2, float* z2, float* t, int code)
{
  float dx, dy, dz;
  float sc;
...
int
OutCode2D(int vport_num, int x, int y)
{
  int code = 0;
...
void
Draw2DSpecialLine(int vport, int x0, int y0, int x1, int y1, int color)
{
  int delta_x, delta_y;
  int x_dir;
  int d, incrE, incrNE;
...
DrawLine(int x0, int y0, int x1, int y1, int color)
{
  ...
  if (y0 < 0)
    y0 = 0; // QUI_InitWindow calls with y0==-1
...
void
DrawArrow2D(int vport, int x0, int y0, int x1, int y1, int col)
{
  float len;
...
void
DrawArrow3D(int vport, float x0, float y0, float z0, float x1, float y1, float z1, int col)
{
  int dx0, dy0, dx1, dy1;
  int tmp, tx, ty;
  float len;
