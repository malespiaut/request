...
static int CheckKey(int key,int wait)
{
   ...
   {
      if (wait)
         while (TestKey(key&0xff)) UpdateMouse();
   }
...
static void CheckCfgErr(int mode)
{
   int i,j;
   cfg_t *c;
...
int ExecCmd(int cmd)
{
   ...
   case CMD_TOGGLE_GRID:
		...
		if (M.display.vport[M.display.active_vport].grid_type == GRID)
...
//		if (M.display.vport[M.display.active_vport].grid_type == ALIGN)
   ...
   case CMD_DEPTHCLIP_DEC:
      status.depth_clip /= 1.2;
      if (status.depth_clip < 10) status.depth_clip = 10;
      redraw=2;
      break;
   ...
   case CMD_GRID_DEC:
  		status.snap_size /= 2;
      if (status.snap_size < 1) status.snap_size = 1;
  		NewMessage("New Snap Size: %d",status.snap_size);
      redraw=2;
      break;
