...
static int
EditCol(char* buf, int col_max, int col_scale)
{
  QUI_window_t* w;
  ...
  for (i = 0; i < 3; i++)
  {
    b_inc[i] = AddButtonPic(w->pos.x + 8, w->pos.y + 32 + 32 * i, B_RAPID, "button_tiny_up");
    b_dec[i] = AddButtonPic(w->pos.x + 8, w->pos.y + 32 + 32 * i + 16, B_RAPID, "button_tiny_down");
  }

  DrawButtons();

  for (i = 0; i < 32; i++)
    DrawLine(w->pos.x + 128, w->pos.y + 48 + i, w->pos.x + 128 + 32, w->pos.y + 48 + i, 255);
  ...
  while (1)
  {
    if (update)
    {
      for (i = 0; i < 3; i++)
      {
        ...
        QUI_DrawStr(w->pos.x + 24, w->pos.y + 32 + 32 * i + 4, BG_COLOR, 0, 0, 0, "%4.0f  ", col[i]);
      }
...
static int
GetFromList(char* buf, char* list)
{
  char temp[256];
...
static void
EditKey(entity_t* e, char* key, const char* v)
{
  class_t* c;
  int i;
  Q_key_t* k;
  ...
  void DrawKeys(void)
  {
    ...
    for (i = 0; i < NUM_DISPLAYED_KEYS; i++)
    {
      ...
      if (i + y < e->numkeys)
      {
        ...
        QUI_DrawStr(k_x1, k_y1 + i * ROM_CHAR_HEIGHT, BG_COLOR, j, 0, 0, "%s", e->key[i + y]);
        QUI_DrawStr(v_x, k_y1 + i * ROM_CHAR_HEIGHT, BG_COLOR, j, 0, 0, "%s", e->value[i + y]);
      }
      else
      {
        if (i + y == key)
        ...
        QUI_DrawStr(k_x1, k_y1 + i * ROM_CHAR_HEIGHT, BG_COLOR, j, 0, 0, "%s", eclass->keys[pkeys[i + y - e->numkeys]].name);
      }
    }
    ...
    QUI_DrawStr(k_x1, nk_y1, BG_COLOR, 14, 0, 0, "%s", n_key);
    QUI_DrawStr(v_x + 4, nk_y1, BG_COLOR, 14, 0, 0, "%s", n_value);
...
void
EditAnyEntity(int nents, entity_t** ents)
{
  int i, j;
  entity_t* Ent;
  entity_t* a;
...
#define MAX_MODELS 512
{
  ...
  void DrawModelList(void)
  {
    ...
    {
      QUI_DrawStr(w->pos.x + 15, w->pos.y + 52 + (j * 14), BG_COLOR, (i == active_ent) ? 14 : 0, 0, 0, "%s", GetKeyValue(ents[i], "classname"));
      QUI_DrawStr(w->pos.x + 300, w->pos.y + 52 + (j * 14), BG_COLOR, (i == active_ent) ? 14 : 0, 0, 0, "%i", brushes[i]);
    }
  }
  ...
  QUI_DrawStr(w->pos.x + 15, w->pos.y + 33, BG_COLOR, 14, 0, 0, "classname");
  QUI_DrawStr(w->pos.x + 300, w->pos.y + 33, BG_COLOR, 14, 0, 0, "Brushes");

  DrawLine(w->pos.x + 10, w->pos.y + 50, w->pos.x + 10, w->pos.y + 294, 4);
  DrawLine(w->pos.x + 10, w->pos.y + 50, w->pos.x + 360, w->pos.y + 50, 4);
  DrawLine(w->pos.x + 10, w->pos.y + 294, w->pos.x + 360, w->pos.y + 294, 8);
  ...
  while (1)
  {
    ...
    if (bp != -1)
    {
      ...
      if (mouse.button & 1)
      {
        ...
        if (InBox(w->pos.x + 10, w->pos.y + 50, w->pos.x + 360, w->pos.y + 294))
        {
          /* Find which model they clicked on */
          for (i = base_ent, j = 0; (j < 17) && (i < models); i++, j++)
          {
            if (InBox(w->pos.x + 10, w->pos.y + 52 + j * 14, w->pos.x + 360, w->pos.y + 66 + j * 14))
              active_ent = i;
          }
