...
P_ClipPoly(void)
{
  ...
  for (i = 0; i < 6; i++)
  {
    ...
    for (j = 0, v = s->pt; j < s->num_pts; j++, v++)
    {
      ...
      if (cb & (v->oc ^ w->oc))
      {
        Clip3D(v->fx, v->fy, v->fz, w->fx, w->fy, w->fz, &inter.x, &inter.y, &inter.z, &scale, (1 << i));
...
static void
TextureAxisFromPlane(plane_t* pln, float vecs[2][3])
{
