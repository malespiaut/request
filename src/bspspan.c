...
static void
AddSpan(int y, int x1, int x2, unsigned char* sl)
{
  int i;
  int prev;
  span_t *sp, *n1, *n2;
...
**************************************************************************/
static void
DrawSpanLight(char* dest, int x1, int x2)
{
  float u, v, w;
  vec3_t r, r2;
  float temp;
...
static void
SpanBSPFaceLight(vec3_dt* pts, int numpts, texture_t* t)
{
...
**************************************************************************/
static void
DrawSpanTC(char* dest, int x1, int x2)
{
  float u, v, w;
  float temp;
  int i;
...
static void
SpanBSPFaceTC(vec3_dt* pts, int numpts, texture_t* t)
{
  float srfloat = 0, slfloat = 0;
  float lslope = 0, rslope = 0;
...
static void
DrawSpanT(char* dest, int x1, int x2)
{
  int i;
  int tx, ty;
  int tdyt;
...
static void
SpanBSPFaceT(vec3_dt* pts, int numpts, texture_t* t)
{
...
static void
DrawSpan(char* dest, int x1, int x2)
{
#ifdef PROFILE
  pix_draw += x2 - x1 + 1;
#endif
...
static void
SpanBSPFace(vec3_dt* pts, int numpts, int color)
{
...
static void
ClipDrawBSPFace(bsp_face_t* F, int color)
{
  int i, j;
  int andcode, orcode;
  float xfloat, yfloat;
  ...
  while (orcode)
  {
    ...
    for (j = 0; j < nsrc; j++)
    {
      ...
      if (pt_in[j] != pt_in[next])
      {
        ...
        Clip3D(p->x, p->y, p->z, p2->x, p2->y, p2->z, &d->x, &d->y, &d->z, &scale, andval);
...
raster:
  ...
  else if (textured == BSP_TEXC)
  {
    PDrawSpan = DrawSpanTC;
    SpanBSPFaceTC(pts[src], nsrc, t);
  }
  else if ((textured == BSP_LIGHT) || (textured == BSP_LIGHTS))
  {
...
static void
DrawBSPFace(bsp_face_t* F, int base_color)
{
...
void
DrawBSPTree(matrix_t m, int vport)
{
  int i;
  float xfloat, yfloat;
  vec3_vt* v;
