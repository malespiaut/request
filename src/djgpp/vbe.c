...
static unsigned int
allocDosMem(int size, unsigned int* segment, unsigned int* selector)
{
  union REGS reg;
...
static void
freeDosMem(unsigned int selector)
{
  union REGS reg;
...
static unsigned int
rmp2pmp(unsigned int addr)
{
  return (((addr & 0xffff0000) >> 12) + (addr & 0xffff));
}

static void
int386rm(unsigned int inter, rminfo* inout)
{
  union REGS reg;
  struct SREGS sreg;
...
int
VBE_getModeInfo(int mode, VBE_modeInfo* infoPtr)
{
  union REGS reg;
  rminfo rmi;
...
void
SV_setPalette(int start, int count, unsigned char* p)
{
  int i;
